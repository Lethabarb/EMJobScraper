...
WORKDIR /src
COPY ["NswLinkExtractor.csproj", "NswLinkExtractor/"]
RUN dotnet restore "NswLinkExtractor/NswLinkExtractor.csproj"

WORKDIR "/src/NswLinkExtractor"
COPY . .
RUN dotnet build "NswLinkExtractor.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "NswLinkExtractor.csproj" \
...
Execute unit tests
```
cd "NswLinkExtractor/test/NswLinkExtractor.Tests"
dotnet test
```

Deploy function to AWS Lambda
```
cd "NswLinkExtractor/src/NswLinkExtractor"
